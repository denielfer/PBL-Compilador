...
class numero_ponto_naoNumero(Exception):
    pass
...
def processar_string(txt:str, chars_list:list[str], rejex:bool = True):
    '''
    ...
    if(NMR != []):
        ...
        print(NMR)
        print(txts)
    for token in processar_blocos(txts[0], chars_list = chars_list, rejex = rejex):
        yield token
    for n,txt in enumerate(txts[1:]):
        ...
        yield (11, NMR[n].strip())
        for token in processar_blocos(txt, chars_list = chars_list, rejex = rejex):
            yield token
...
def __separa_delimitadores__(chars_list:list[str],txt:str):
    '''
        Função que faz o split dos separadores, exceto '.'
        retorno: Lista de strings separadas, Lista dos separadores que separaram a string
    '''
    txts = [txt]
    token_delimitadores = []
    for delimitador in chars_list: # para cada delimitador
        for n in range(len(txts))[::-1]: 
            ...
            for txt in new_txt[-2::-1]:
                ...
                txts.insert(n, txt)
    return txts,token_delimitadores

def __gera_tokens__(txts,token_delimitadores,chars_list,rejex):
    erro_ponto = False
    erro_ponto_text = ''
    for txt in txts: # depois de lidarmos com todos os delimitadores precisamos processar cada string 
        if erro_ponto:
            ...
            erro_ponto_text+=delimitador[0]
            # print(f'\ttratando erro {erro_ponto_text}')
            # print(delimitador)
            if len(delimitador) >1:
                txt = delimitador[1:]+txt
            if txt == '':
                yield (11, erro_ponto_text)
                return
            # print(txt)
            t,d = __separa_delimitadores__(chars_list,txt)
            # print('\t',t,d)
            for token in __gera_tokens__(t,d,chars_list,rejex):
                yield token
            continue
        try:
            for token in i_pr_n.ação(txt, rejex = rejex ):
                yield token
        except numero_ponto_naoNumero as e:
            # print(f'erro de numero. encontrado: {e}')
            if len(token_delimitadores) == 0:
                # print('sem delimitador')
                raise e
            erro_ponto_text = str(e)
            erro_ponto = True
            continue
...
def get_token(txt:str, chars_list:list[str], rejex:bool = True):
    '''
    ...
    if chars_list == []: # se a lista de separadores ta vazia fazemos analize de IDE PRE NRO E TMF
        ...
        for token in i_pr_n.ação(txt, rejex = rejex):
            yield token
        return
    txts,token_delimitadores = __separa_delimitadores__(chars_list,txt)
    for token in __gera_tokens__(txts,token_delimitadores,chars_list,rejex):
        yield token
...
class i_pr_n(): # identificadores, palavras reservadas e numeros
    ...
    palavras_reservadas = ["variables", "const", "class", "methods","objects", 
                           ...
                           "true", "false"]
    def ação(txt:str, rejex:bool = True):
        txt = txt.strip()  # remove ' ' do inicio e fim
        func = i_pr_n.__monta_token__ if rejex else i_pr_n.__monta_token_regex__
        ...
        if txt[n] in ascii_letters: # verifica indentificador ou palavra reservada
            ...
            for n, char in enumerate(txt[n+1:]):
                if char in digits_temp:
                    ...
                    elif erro2 and not erro:
                        # print(f'identificador erro numero: {identificador}')
                        raise numero_ponto_naoNumero(identificador)
                    else:
    ...
    re_NMR =  r"[\s\b]\d+\.[.\+\=\-\!\&\|\*\/\<\>\[\]\{\}\(\)\;\,][^(\+\=\-\!|(\&\&)|(\|\|)|\*\(\/\)<\>\[\]\{\}\(\)\;\,\s)]*?(?:{escape(string)})"
...
if __name__ == '__main__':
    ...
    # s = ' 6.a35#.99; 2.+231{23as+ 2.{2132}31 }'
    # print(s) # aparece 2 linhas porque o \n é usado como quebra de linha
    # for a in processar_string(s, prioridade):
    #     print(a)
